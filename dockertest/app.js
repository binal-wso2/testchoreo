...
// Function to get the container's network range
function getNetworkRange(callback) {
  exec("ip -o -f inet addr show | awk '/scope global/ {print $4}'", (error, stdout, stderr) => {
    if (error) {
      console.error(`Error fetching network range: ${stderr}`);
      callback(null);
    } else {
      const networkRange = stdout.trim().split('\n')[0]; // Use the first network range
      callback(networkRange);
    }
  });
}
...
app.get('/auto-scan', (req, res) => {
  getNetworkRange((networkRange) => {
    if (!networkRange) {
      return res.status(500).json({ error: 'Failed to determine network range' });
    }
    ...
    exec(hostDiscoveryCommand, { maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
      ...
      while ((match = hostsRegex.exec(stdout)) !== null) {
        ...
        const scanPromise = new Promise((resolve) => {
          ...
          const scanCommand = `nmap --open ${ipAddress}`;
